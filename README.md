...
This repository implements an analysis pipeline for TNBC FLIM data, including preprocessing and nuclei segmentation, feature extraction, classification models for predicting neoadjuvant chemotherapy resistance and spatial analysis [(see Repository overview)](#5-repository-overview). It also provides reproducible notebooks to recreate the paper’s results alongside scripts for [Direct Script Run](#6-direct-script-run) and usage examples [(see Notebook Examples)](#7-notebook-examples) that enable running individual stages of the pipeline.
