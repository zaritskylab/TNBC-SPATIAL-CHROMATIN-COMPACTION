...
DEFAULT_TICK_PARAMS = {
    'axis': 'both', 'width': DEFAULT_LINE_WIDTH, 'length': 10
}
...
DEFAULT_CONTINUOUS_CMAP = matplotlib.pyplot.cm.get_cmap('cividis')
DEFAULT_DISCRETE_CMAP = sns.color_palette("colorblind")


def set_titles_and_labels(
    ax: matplotlib.axes.Axes, title: str = '', xlabel: str = '',
    ylabel: str = '', fontdict: Dict[str, Union[str,
                                                float]] = DEFAULT_LABEL_FONTDICT
):
  """
...
def customize_legend(
    ax: matplotlib.axes.Axes, loc: str = 'upper left',
    legend_prop: Dict[str, str] = DEFAULT_LEGEND_PROP,
    line_width: float = DEFAULT_LEGEND_LINE_WIDTH,
    bbox_to_anchor: tuple[float, float] = None
):
  """
  Create and customize the legend for the plot.

  Args:
      ax (matplotlib.axes.Axes): The axes object of the subplot. 
      loc (str, optional): The location of the legend. Defaults to
          'upper left'. 
      legend_prop (Dict[str, str], optional): Properties of the legend text.
          Defaults to DEFAULT_LEGEND_PROP.
      line_width (float, optional): Width of the lines in the legend. 
          Defaults to DEFAULT_LEGEND_LINE_WIDTH
      bbox_to_anchor (tuple[float, float], optional): The position of the
          legend relative to the axes. Defaults to None.
  """
  leg = ax.legend(loc=loc, prop=legend_prop, bbox_to_anchor=bbox_to_anchor)
  for line in leg.get_lines():
    line.set_linewidth(line_width)
    for text in leg.get_texts():
      text.set_weight(legend_prop.get('weight'))
      text.set_color(legend_prop.get('color', DEFAULT_COLOR))
      text.set_fontsize(legend_prop.get('size', DEFAULT_FONT_SIZE))
