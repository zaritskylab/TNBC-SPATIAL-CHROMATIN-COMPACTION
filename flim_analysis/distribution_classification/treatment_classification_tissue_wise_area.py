...
def preprocess_area_df(
    metadata_df: pd.DataFrame, area_distribution: pd.DataFrame
) -> pd.DataFrame:
  """
  Preprocess the area DataFrame.

  Args:
    metadata_df (pd.DataFrame): The metadata DataFrame.

  Returns:
    pd.DataFrame: The preprocessed area DataFrame.

  """
  area_distribution["leap_id"] = area_distribution["leap_ID"].map(
      lambda x: f"Leap{str(x).zfill(3)}"
  )
  area_distribution["response"] = area_distribution["leap_id"].map(
      lambda leap_id: metadata_df[metadata_df["leap_id"] == leap_id]["response"]
      .values[0].lower()
  )
  area_distribution["response"] = area_distribution["response"].map(
      lambda x: "non responder" if x == "non-responder" else "responder"
  )
  area_distribution["analysis_id"] = area_distribution["leap_id"].map(
      lambda leap_id: metadata_df[metadata_df["leap_id"] == leap_id][
          "analysis_id"].values[0]
  )
  area_distribution.drop(columns=["categories", "leap_ID"], inplace=True)
  return area_distribution


def rename_area_columns(
    area_distribution: pd.DataFrame, area_columns: list
) -> tuple:
  """
  Rename area columns in the DataFrame.

  Args:
    area_distribution (pd.DataFrame): The area distribution DataFrame.
    area_columns (list): List of area columns to rename.
  ...
  for col in area_columns:
    new_col_name = col.replace("area_", "")
    ...
    area_distribution.rename(columns={col: new_col_name}, inplace=True)
    new_area_columns.append(new_col_name)
  return area_distribution, new_area_columns


if __name__ == '__main__':
  # Parse command-line arguments
  parser = argparse.ArgumentParser(
      description="FLIM tissue wise classification pipeline using area features"
  )
  parser.add_argument(
  ...
  parser.add_argument(
      ...
      "features_area_distribution_data_max_val_100.0_bins_amount_18_bin_range_5.556.csv",
      help="Name of the CSV file containing area distribution features"
  )
  args = parser.parse_args()

  # Print message
  print(f"Running FLIM tissue-wise classification pipeline using area features...")

  # Load metadata
  metadata_df = load_metadata(const.RCB_FILE)

  # Load area distribution data
  ...
  area_distribution = pd.read_csv(distribution_csv_path)
  area_distribution = preprocess_area_df(
      metadata_df, area_distribution
  )

  # Get area columns
  not_area_columns = [
      col for col in area_distribution.columns if 'area' not in col
  ]

  area_columns = [
      col for col in area_distribution.columns if 'area' in col
  ]

  # Rename area columns
  area_distribution, area_columns = rename_area_columns(
      area_distribution, area_columns
  )

  # Create data for classification
  X = area_distribution.drop(columns=not_area_columns).values
  y = (area_distribution["response"].values != "non responder").astype(int)
  leap_ids = area_distribution["leap_id"].values
  group_ids = area_distribution["analysis_id"].values
  feature_names = area_distribution.drop(
      columns=not_area_columns
  ).columns

  # Create output directory
  cv_strategy = "loocv"
  save_dir = (
      Path(const.DISTRIBUTION_RESULTS_BASE_DIR) / "classification_results_using_area" /
