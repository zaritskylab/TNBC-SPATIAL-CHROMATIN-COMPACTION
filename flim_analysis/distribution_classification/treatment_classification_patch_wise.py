...
import re
from flim_analysis.distribution_classification.binary_classifier_trainer import WeakSupervisionBinaryClassifierTrainer
...
def preprocess_lifetime_df(
  ...
  lifetime_distribution.rename(columns={"patch_ID": "patch_id"}, inplace=True)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      "features_lifetime_distribution_data_patches_size_1000_overlap_0.75_max_val_13_bins_amount_18_bin_range_0.73.csv",
  ...
  instance_ids = lifetime_distribution["patch_id"].values
  ...
  save_dir = (
      Path(const.DISTRIBUTION_RESULTS_PATCH_DIR) / feature_type /
      f"size_{args.patch_size}_overlap_{overlap}" / model_name /
      cv_strategy / f"{bin_amount}_bins"
  )
  ...
  # Run classification
  trainer = WeakSupervisionBinaryClassifierTrainer(
      X, y, patient_ids=group_ids, sample_ids=leap_ids,
      instance_ids=instance_ids, feature_names=feature_names,
      num_seeds=args.n_seeds, num_permutations=args.n_permutations,
      model_type=model_name, output_dir=save_dir, tune_hyperparameters=True,
      primary_seed=PRIMARY_SEED, cv_strategy=cv_strategy,
      optuna_cv_strategy='stratified_kfold', optuna_cv_n_splits=5
  ).run()
